...
import { Send, Copy, ThumbsUp, ThumbsDown, Paperclip, Brain } from 'lucide-react'
...
interface ExtendedMessage {
  ...
  tools?: Array<{ type: string; state: 'input-streaming' | 'input-available' | 'output-available' | 'output-error'; input: any; output: any; toolCallId: string }>
  isStreaming?: boolean
}
...
function ChatPage() {
  ...
  const extendedMessages: ExtendedMessage[] = messages.map(msg => ({
    ...msg,
    timestamp: new Date(),
    // Add mock reasoning and sources for assistant messages to showcase UI
  ...
  return (
    <div className="flex h-screen flex-col">
          ...
          <ChatContainerContent className="space-y-4 p-4 h-full overflow-y-auto min-h-0">
            ...
            {connectionStatus === 'error' && (
              <div className="bg-destructive/10 border border-destructive text-destructive px-4 py-2 rounded-lg">
                ...
                <p className="text-sm">{connectionError || 'Unable to connect to AI service'}</p>
                <p className="text-xs mt-1">Please check if the server is running and environment variables are configured.</p>
              </div>
            )}
            ...
            {extendedMessages.map((message) => (
              <Message key={message.id} className="flex gap-3">
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  ...
                  {/* Message Content */}
                  <MessageContent markdown className="prose prose-sm max-w-none">
                    {message.content}
                  </MessageContent>
      ...
      {uploadedFiles.length > 0 && (
        <div className="shrink-0 border-t bg-muted/30 p-4">
          <div className="mb-2 text-sm text-muted-foreground font-medium">Uploaded files:</div>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="p-4">
          <FileUpload onFilesAdded={handleFilesAdded} multiple accept=".txt,.md,.json,.csv,.pdf">
            <form onSubmit={handleCustomSubmit}>
              ...
              >
              <PromptInputTextarea
                placeholder="Type your message here or drop files to upload..."
            ...
            <FileUploadContent>
              <div className="text-center p-8">
                ...
                <p className="text-sm text-muted-foreground">Supports .txt, .md, .json, .csv, .pdf files</p>
              </div>
            </FileUploadContent>
          </FileUpload>
        </div>
